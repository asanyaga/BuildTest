...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class DistributorSalesmanImporterService:BaseImporterService,IDistributorSalesmanImporterService
    {
        ...
        {
            var exists = _context.tblUsers.FirstOrDefault(p => p.Code == distributorSalesmanImport.Code);
            Guid id = exists != null ? exists.Id : Guid.NewGuid();

            var costCentre =_context.tblCostCentre.FirstOrDefault(p => p.Cost_Centre_Code == distributorSalesmanImport.Code);
            if (costCentre == null) { mappingUserValidationList.Add(string.Format("Invalid DistributorSalesman Code {0}", distributorSalesmanImport.Code)); }
        ...
        {
            var exists = _context.tblCostCentre.FirstOrDefault(p => p.Cost_Centre_Code == distributorSalesmanImport.Code && p.CostCentreType == (int)CostCentreType.DistributorSalesman);
            Guid id = exists != null ? exists.Id : Guid.NewGuid();

            var distributor =
                _context.tblCostCentre.FirstOrDefault(
                    p =>
                    p.Cost_Centre_Code == distributorSalesmanImport.DistributorCode &&
                    p.CostCentreType == (int) CostCentreType.Distributor) ;
            if (distributor == null) { mappingvalidationList.Add(string.Format("Invalid Distributor Code {0}", distributorSalesmanImport.DistributorCode)); }
