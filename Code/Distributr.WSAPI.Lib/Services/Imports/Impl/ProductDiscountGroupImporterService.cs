...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class ProductDiscountGroupImporterService:BaseImporterService,IProductDiscountGroupImporterService
    {
        ...
        protected ProductGroupDiscount Map(ProductDiscountGroupItemImport productDiscountGroupItemImport, List<string> mappingvalidationList)
        {
            ...
            if (discountGroup==null)
            {
                mappingvalidationList.Add(string.Format("Invalid Discount Group Code {0}", productDiscountGroupItemImport.ProductDiscountGroupCode));
                return null;
            }
            if (!_productList.ContainsValue(productDiscountGroupItemImport.ProductCode.ToLower()))
            {
                mappingvalidationList.Add(string.Format("Invalid Product Code {0}", productDiscountGroupItemImport.ProductCode));
                return null;
            }
            var productId =_productList.FirstOrDefault(k => k.Value.ToLower() == productDiscountGroupItemImport.ProductCode.ToLower()).Key;


            tblProductDiscountGroup exists = _context.tblProductDiscountGroup.FirstOrDefault(n => n.DiscountGroup == discountGroup.Id && n.Quantity == productDiscountGroupItemImport.Quantity && n.ProductRef == productId);
