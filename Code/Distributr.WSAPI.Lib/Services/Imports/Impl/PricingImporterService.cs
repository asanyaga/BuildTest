...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    ...
    public class PricingImporterService : BaseImporterService, IPricingImporterService
    {
        ...
        protected ProductPricing Map(PricingImport pricingImport, List<string> mappingvalidationList )
        {
            ...
            var product = _context.tblProduct.FirstOrDefault(p => p.ProductCode == pricingImport.ProductCode);
            if (product == null) { mappingvalidationList.Add(string.Format("Invalid Product Code {0}", pricingImport.ProductCode)); }
            ...
            if (pricingTier == null) { mappingvalidationList.Add(string.Format("Invalid PricingTier Code {0}", pricingImport.ProductPricingTierCode)); }
