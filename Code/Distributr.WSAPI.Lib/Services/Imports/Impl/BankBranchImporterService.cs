...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class BankBranchImporterService:BaseImporterService,IBankBranchImporterService
    {
        ...
        private BankBranch Map(BankBranchImport bankBranchImport, List<string> mappingvalidationList)
        {
            ...
            var bank = _context.tblBank.FirstOrDefault(p => p.Code == bankBranchImport.BankCode);

            if (bank == null) { mappingvalidationList.Add(string.Format("Invalid Bank Code {0}", bankBranchImport.BankCode)); }
