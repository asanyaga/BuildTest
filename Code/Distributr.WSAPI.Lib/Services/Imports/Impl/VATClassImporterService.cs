...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class VATClassImporterService:BaseImporterService,IVATClassImporterService
    {
        ...
        {
            var exists = _context.tblVATClass.FirstOrDefault(p => p.Name == vatClassImport.Name);
            ...
            var classItemExists = _context.tblVATClassItem.FirstOrDefault(p => p.VATClassID == id && p.Rate==vatClassImport.CurrentRate);
