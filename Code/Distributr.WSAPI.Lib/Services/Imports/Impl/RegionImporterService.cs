...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class RegionImporterService : BaseImporterService, IRegionImporterService
    {
        ...
        {
            var exists = _context.tblRegion.FirstOrDefault(p => p.Name == regionImport.Code);
            Guid id = exists != null ? exists.id : Guid.NewGuid();

            var country = _context.tblCountry.FirstOrDefault(p => p.Code == regionImport.CountryCode);
            if(country==null)
            {mappingvalidationList.Add(string.Format("Invalid Country Code {0}",regionImport.CountryCode));}
