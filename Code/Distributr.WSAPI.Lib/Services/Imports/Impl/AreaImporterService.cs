...
namespace Distributr.WSAPI.Lib.Services.Imports.Impl
{
    public class AreaImporterService:BaseImporterService,IAreaImporterService
    {
        ...
        {
            var exists = _context.tblArea.FirstOrDefault(p => p.Name == areaImport.Name);
            Guid id = exists != null ? exists.id : Guid.NewGuid();


            var regionId = _context.tblRegion.Where(p => p.Name == areaImport.RegionCode).Select(p=>p.id).FirstOrDefault();
