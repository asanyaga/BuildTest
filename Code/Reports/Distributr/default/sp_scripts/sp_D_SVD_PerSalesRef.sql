DROP PROCEDURE [dbo].[sp_D_SVD_PerSalesRef]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[sp_D_SVD_PerSalesRef]
(
...
GROUP BY ctry.id,ctry.Name,reg.id,reg.Name,distributor.Id,distributor.Name,salesman.Id,salesman.Name,outlet.Id,outlet.Name,sales.id,sales.DocumentReference,sales.DocumentDateIssued,sales.SaleDiscount 
HAVING SUM(sales.SaleDiscount) > 0
ORDER BY sales.DocumentDateIssued DESC    


--  EXEC  [dbo].[sp_D_SVD_PerSalesRef] @countryId='ALL' ,@regionId='ALL' ,@distributorId='ALL' ,@salesmanId='ALL' ,@outletId='ALL' ,@startDate='2014-05-22',@endDate='2014-05-22'
