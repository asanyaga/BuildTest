...
DROP PROCEDURE sp_D_PaymentSummary_Per_Salesman_Outstanding
GO
SET ANSI_NULLS ON
GO
...
GO
CREATE procedure sp_D_PaymentSummary_Per_Salesman_Outstanding
(
...
--		--AND CONVERT(NVARCHAR(26),sales.DocumentDateIssued,23) between @startDate and @endDate --ommmited on 2april2015 as requsted by CTO and CSO for F
...
--		SUM(saleItems.Quantity * (saleItems.Value + saleItems.Vat)) AS InvoiceValue, 
--		sales.SaleDiscount AS discount,
...
--			InvoiceValue,
--			discount
--HAVING case when (InvoiceValue - (discount + SUM(CashPaid) + SUM(ChequePaid) + SUM(MmoneyPaid))) % 1 < 0.04 then floor(InvoiceValue - (discount + SUM(CashPaid) + SUM(ChequePaid) + SUM(MmoneyPaid))) else ceiling(InvoiceValue - (discount + SUM(CashPaid) + SUM(ChequePaid) + SUM(MmoneyPaid))) end > 0

--GO

-- Exec sp_D_PaymentSummary_Per_Salesman_Outstanding @startDate = '2014-02-18',@endDate='2015-2-18', @salesmanId = 'ALL',@outletId = 'ALL',@distributorId='ALL',@routeId='ALL'

-- Exec sp_D_PaymentSummary_Per_Salesman_Outstanding @endDate='2015-3-31',@salesmanId = 'ALL',@outletId = 'ALL',@distributorId='ALL',@routeId='ALL'
...
-- Exec sp_D_PaymentSummary_Per_Salesman_Outstanding  @startDate = '2015-03-01',@endDate = '2015-03-31', @salesmanId = 'ALL',@outletId = 'ALL',@distributorId='ALL',@routeId='ALL'
