--/****** Object:  StoredProcedure [dbo].[sp_D_Inventory_StockSummary_PerProduct]    Script Date: 07/24/2013 08:51:32 ******/
DROP PROCEDURE [dbo].[sp_D_Inventory_StockSummary_PerProduct]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_D_Inventory_StockSummary_PerProduct]
(
...
FROM       dbo.tblProductBrand INNER JOIN
           ...
           dbo.tblInventory ON dbo.tblCostCentre.Id = dbo.tblInventory.WareHouseId
           ON dbo.tblProduct.id = dbo.tblInventory.ProductId
           ON dbo.tblProductBrand.id = dbo.tblProduct.BrandId inner join   
...
WHERE     (dbo.tblInventory.Balance >= 0) AND (dbo.tblCostCentre.CostCentreType = 4)
    ...
    --   Exe [dbo].[sp_D_Inventory_StockSummary_PerProduct] @brandId='ALL',@distributorId='ALL',@salesmanId='ALL',@productId='ALL',@packagingId='ALL',@Brand='ALL',@Product='ALL',@productTypeId='ALL'
