---RETURNABLES ORDERS
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_D_CrateOrders_PerDistributor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_D_CrateOrders_PerDistributor]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_D_CrateOrders_PerDistributor]
(
@startDate as date,
@endDate as date,
...
SELECT cntry.id as CountryId,cntry.Name as CountryName,
	   ...
	   salesman.Id SalesmanId,salesman.Name SalesmanName,
...
SELECT CountryId,CountryName,
	   ...
	   SUM(TotalVat) TotalVat,
	   SUM(ProductDiscount) ProductDiscount,
   	   CAST(ROUND((SUM(TotalSales) / ReturnablePackCapacity),1) AS INT) NoOfCrates,
	   ReturnablePackCapacity ,
	   CAST((SUM(TotalSales) % ReturnablePackCapacity) AS INT)  ExtraBottle,
	   SUM(GrossAmount) GrossAmount,
	   SUM(NetTruncate) NetTruncate,
	   SUM(NetRoundOff) NetRoundOff,
	   SUM(GrossRoundOff) GrossRoundOff
FROM CrateSales_CTE2
GROUP BY CountryId,CountryName,
         RegionId,RegionName,
		 DistributorId,DistributorName,
		 --SalesmanId,SalesmanName,
         ProductName,ReturnablePackCapacity

-- EXEC [dbo].[sp_D_CrateOrders_PerDistributor] @startDate='2015-01-01',@endDate='2015-01-16',@countryId=' ALL',@regionId=' ALL',@distributorId=' ALL'
