IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_commodityProducerServicesByFarm')
   exec('CREATE PROCEDURE [sp_A_commodityProducerServicesByFarm] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_commodityProducerServicesByFarm
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Farm,ActivityName,Service,SUM(Cost) AS Cost,SUM(NoOfActivities) AS NoOfActivities
FROM Producer_CTE

GROUP BY Farm,ActivityName,Service

-- EXEC sp_A_commodityProducerServicesByFarm @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
