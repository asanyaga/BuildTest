DROP PROCEDURE [dbo].[sp_A_DB_CommodityCenterReceivedAndStoredWeightAnalysis]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[sp_A_DB_CommodityCenterReceivedAndStoredWeightAnalysis]
(
...
if  @regionId='ALL'  begin set @regionId='%' end
  ...
  SELECT DISTINCT 
          ...
          item.Weight AS ReceivedAndStoredWeight
   ...
   AND (doc.DocumentTypeId = 15 or doc.DocumentTypeId = 17)
   AND convert(nvarchar(26),doc.DocumentDate,23) between @startDate and @endDate 
   
   
 -- Exec [dbo].[sp_A_DB_CommodityCenterReceivedAndStoredWeightAnalysis] @startDate='2013-10-16',@endDate='2013-10-16',@regionId='ALL'
