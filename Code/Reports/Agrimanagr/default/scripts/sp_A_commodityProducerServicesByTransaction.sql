IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_commodityProducerServicesByTransaction')
   exec('CREATE PROCEDURE [sp_A_commodityProducerServicesByTransaction] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_commodityProducerServicesByTransaction
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT	DISTINCT dbo.tblActivityDocument.Id AS ActivityId,
		...
		tblCommodityOwner.FirstName + ' ' + tblCommodityOwner.Surname AS Farmer,
		tblCommodityProducer.Name AS Farm
...
SELECT ActivityReference,ActivityName,Service,SUM(Cost) AS Cost,SUM(NoOfActivities) AS NoOfActivities
FROM Producer_CTE

GROUP BY ActivityReference,ActivityName,Service

-- EXEC sp_A_commodityProducerServicesByTransaction @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
