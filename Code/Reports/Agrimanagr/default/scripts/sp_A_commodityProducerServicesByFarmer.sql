IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_commodityProducerServicesByFarmer')
   exec('CREATE PROCEDURE [sp_A_commodityProducerServicesByFarmer] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_commodityProducerServicesByFarmer
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Farmer,ActivityName,Service,SUM(Cost) AS Cost,SUM(NoOfActivities) AS NoOfActivities
FROM Producer_CTE

GROUP BY Farmer,ActivityName,Service

-- EXEC sp_A_commodityProducerServicesByFarmer @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
