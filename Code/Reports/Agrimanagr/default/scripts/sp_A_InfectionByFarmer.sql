IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_InfectionByFarmer')
   exec('CREATE PROCEDURE [sp_A_InfectionByFarmer] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_InfectionByFarmer
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT FarmerName,Infection,InfectionRate,SUM(NoOfActivities) AS NoOfActivities
FROM Infection_CTE

GROUP BY FarmerName,Infection,InfectionRate

-- EXEC sp_A_InfectionByFarmer @StartDate='2015-01-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
