IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_InfectionByFactory')
   exec('CREATE PROCEDURE [sp_A_InfectionByFactory] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_InfectionByFactory
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Factory,Infection,InfectionRate,SUM(NoOfActivities) AS NoOfActivities
FROM Infection_CTE

GROUP BY Factory,Infection,InfectionRate

-- EXEC sp_A_InfectionByFactory @StartDate='2015-01-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
