IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_ServiceProviderByFactory')
   exec('CREATE PROCEDURE [sp_A_ServiceProviderByFactory] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_ServiceProviderByFactory
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Factory,ServiceProvideName,SUM(NoOfActivities) AS NoOfActivities
FROM Service_CTE
GROUP BY Factory,ServiceProvideName

-- EXEC sp_A_ServiceProviderByFactory @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
