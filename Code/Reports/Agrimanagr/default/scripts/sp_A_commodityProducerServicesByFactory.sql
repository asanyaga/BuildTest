IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_commodityProducerServicesByFactory')
   exec('CREATE PROCEDURE [sp_A_commodityProducerServicesByFactory] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_commodityProducerServicesByFactory
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Factory,ActivityName,Service,SUM(Cost) AS Cost,SUM(NoOfActivities) AS NoOfActivities
FROM Producer_CTE

GROUP BY Factory,ActivityName,Service

-- EXEC sp_A_commodityProducerServicesByFactory @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
