IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_DepositorWarehouse')
   exec('CREATE PROCEDURE [sp_A_DepositorWarehouse] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_DepositorWarehouse
(
@StartDate AS DATE,
@EndDate AS DATE,
...
;WITH Depositor_CTE AS (
SELECT DISTINCT doc.DocumentDate,
	doc.DocumentReference,
	clerk.Name AS Clerk,
	hub.Name AS Warehouse,
	hub.Id AS WarehouseId,
	tblRegion.Name AS Location, 
	tblCommodityOwner.FirstName AS Depositor,
	1 AS NoOfDeppositors,
...
SELECT Warehouse,WarehouseId,Commodity,Grade,SUM(NoOfDeppositors) AS NoOfDeppositors,SUM(NetWeight) AS NetWeight
FROM Depositor_CTE
GROUP BY Warehouse,WarehouseId,Commodity,Grade	
END;

-- EXEC sp_A_DepositorWarehouse @StartDate='2015-06-01',@EndDate='2015-07-23',@HubId='ALL',@ClerkId='ALL',@CountryId='ALL',@LocationId='ALL',@DepositorId='ALL',@StoreId='ALL',@CommodityId='ALL',@GradeId='ALL'
