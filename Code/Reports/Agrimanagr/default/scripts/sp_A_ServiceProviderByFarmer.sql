IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_ServiceProviderByFarmer')
   exec('CREATE PROCEDURE [sp_A_ServiceProviderByFarmer] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_ServiceProviderByFarmer
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT FarmerName,ServiceProvideName,SUM(NoOfActivities) AS NoOfActivities
FROM Service_CTE
GROUP BY FarmerName,ServiceProvideName

-- EXEC sp_A_ServiceProviderByFarmer @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
