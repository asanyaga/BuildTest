IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_InfectionByFarm')
   exec('CREATE PROCEDURE [sp_A_InfectionByFarm] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_InfectionByFarm
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Farm,Infection,InfectionRate,SUM(NoOfActivities) AS NoOfActivities
FROM Infection_CTE

GROUP BY Farm,Infection,InfectionRate

-- EXEC sp_A_InfectionByFarm @StartDate='2015-01-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
