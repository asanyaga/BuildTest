IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_DepositorCountry')
   exec('CREATE PROCEDURE [sp_A_DepositorCountry] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_DepositorCountry
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Country,CountryId,Commodity,Grade,SUM(NoOfDeppositors) AS NoOfDeppositors,SUM(NetWeight) AS NetWeight
FROM Depositor_CTE
GROUP BY Country,CountryId,Commodity,Grade	
END;

-- EXEC sp_A_DepositorCountry @StartDate='2015-06-01',@EndDate='2015-07-23',@HubId='ALL',@ClerkId='ALL',@CountryId='ALL',@LocationId='ALL',@DepositorId='ALL',@StoreId='ALL',@CommodityId='ALL',@GradeId='ALL'
