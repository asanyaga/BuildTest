IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_ServiceProviderByFarm')
   exec('CREATE PROCEDURE [sp_A_ServiceProviderByFarm] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_ServiceProviderByFarm
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT Farm,ServiceProvideName,SUM(NoOfActivities) AS NoOfActivities
FROM Service_CTE
GROUP BY Farm,ServiceProvideName

-- EXEC sp_A_ServiceProviderByFarm @StartDate='2015-06-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
