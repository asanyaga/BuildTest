IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_SeasonsByCentre')
   exec('CREATE PROCEDURE [sp_A_SeasonsByCentre] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_SeasonsByCentre
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT CentreName,ActivityName,Season,SUM(NoOfActivities) AS NoOfActivities
FROM Seasons_CTE
GROUP BY CentreName,ActivityName,Season


-- EXEC sp_A_SeasonsByCentre @StartDate='2015-01-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
