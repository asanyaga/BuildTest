IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'sp_A_ActivityByTransaction')
   exec('CREATE PROCEDURE [sp_A_ActivityByTransaction] AS BEGIN SET NOCOUNT ON; END')
GO
Alter PROCEDURE sp_A_ActivityByTransaction
(
@StartDate AS DATE,
@EndDate AS DATE,
...
SELECT	DISTINCT dbo.tblActivityDocument.Id AS ActivityId,
		...
		dbo.tblUsers.UserName AS Clerk
...
SELECT ActivityReference,ActivityName,COUNT(ActivityName) AS NoOfActivities
FROM Activity_CTE

GROUP BY ActivityReference,ActivityName,ActivityDate

ORDER BY ActivityDate DESC

-- EXEC sp_A_ActivityByTransaction @StartDate='2014-01-01',@EndDate='2015-07-15',@HubId='ALL',@RouteId='ALL',@CentreId='ALL',@FarmerId='ALL',@FarmId='ALL',@ActivityId='ALL',@ClerkId='ALL'
