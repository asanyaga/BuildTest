...
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'Distributor')							-- Change Database Name from Distributor to ??
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Distributor', 
		@enabled=1, 
		...
		@owner_login_name=N'RONNYPC\ronald.kudoyi', @job_id = @jobId OUTPUT								-- Change to Login name of SQL server
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
