...
{
    public class OutletTypeImportService : MasterDataImportServiceBase, IOutletTypeImportService
    {
        private readonly IOutletTypeRepository _repository;
        private readonly IDTOToEntityMapping _mappingService;
        private readonly CokeDataContext _ctx;

        public OutletTypeImportService(IOutletTypeRepository repository, IDTOToEntityMapping mappingService, CokeDataContext ctx)
        {
        ...
        public async Task<MasterDataImportResponse> ValidateAndSaveAsync(IEnumerable<ImportEntity> imports)
        {
            return await Task.Run<MasterDataImportResponse>(async () =>
            {
                ...
                foreach (var dto in dtos)
                {
                    ...
                    if (res.IsValid)
                    {
                        _repository.Save(res.Entity as OutletType, true);
                    }
                    vResults.ValidationResults.Add(res);
                    index++;
                }
        ...
        private async Task<ImportValidationResultInfo> MapAndValidate(OutletTypeDTO dto, int index)
        {
            return await Task.Run(() =>
            {
                var entity = _mappingService.Map(dto);
                var exist = _ctx.tblOutletType.FirstOrDefault(p => p.Code.ToLower() == dto.Code.ToLower()||p.Name.ToLower()==dto.Name.ToLower());
        ...
        private async Task<IEnumerable<OutletTypeDTO>> ConstructDtOs(IEnumerable<ImportEntity> entities)
        {
            var items = new List<OutletTypeDTO>();
            return await Task.Run(() =>
            {
                items.AddRange(entities.Select(n => n.Fields).Select(row => new OutletTypeDTO()
                {
