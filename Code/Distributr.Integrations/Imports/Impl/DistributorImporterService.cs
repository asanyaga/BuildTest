...
namespace Distributr.Integrations.Imports.Impl
{
    public class DistributorImporterService : BaseImporterService, IDistributorImporterService
    {
        ...
        {
            var exists = Queryable.FirstOrDefault(_context.tblCostCentre, p => p.Cost_Centre_Code == distributorImport.Code);
            Guid id = exists != null ? exists.Id : Guid.NewGuid();


            var regionId = Queryable.Where(_context.tblRegion, p => p.Name == distributorImport.RegionName).Select(p => p.id).FirstOrDefault();
            var region = _regionRepository.GetById(regionId);
            if(region==null){mappingvalidationList.Add(string.Format((string) "Invalid Region Code {0}",(object) distributorImport.RegionName));}
