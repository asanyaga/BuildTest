...
namespace Distributr.Integrations.Imports.Impl
{
    public class SaleProductImporterService : BaseImporterService, ISaleProductImporterService
    {
        ...
        {
            var exists = Queryable.FirstOrDefault(_context.tblProduct, p => p.ProductCode == saleProductImport.Code);
            Guid id = exists != null ? exists.id : Guid.NewGuid();

            var vatClassExists = Queryable.FirstOrDefault(_context.tblVATClass, p => p.Name == saleProductImport.VATClass);
            ...
            var productBrandExists =
                Queryable.FirstOrDefault(_context.tblProductBrand, p => p.code == saleProductImport.ProductBrandCode);
            ...
            var productFlavourExists =
              Queryable.FirstOrDefault(_context.tblProductFlavour, p => p.code == saleProductImport.ProductFlavour);
            ...
            if (productFlavourExists != null)
             ...
             var productPackagingExists =
              Queryable.FirstOrDefault(_context.tblProductPackaging, p => p.code == saleProductImport.ProductPackaging);
